...
int IRDetectorPin3 = 6; // chân thứ 3
...
boolean photoGate3; 
boolean lastState3 = 0;

int eventNum = 1; // Số lần vật đi qua cổng quang
// Thời điểm vật đi qua mỗi cổng quang ứng với số lần đó kể từ khi bắt đầu bật mạch
long eventTime,eventTime2,eventTime3; 
...
void setup()
{
  ...
  pinMode(IRDetectorPin3, INPUT_PULLUP);
...
void loop()
{
  ...
  if (photoGate != lastState && photoGate != 0)  // Nếu có sự thay đổi trạng thái của cổng quang:
    {
      ...
      //setMillis(new_val);
      delay(20); // Thời gian cách mỗi lần đo 
    }
  ...
  photoGate3 = digitalRead(IRDetectorPin3);
  if (photoGate3 != lastState3 && photoGate3 != 0)
    {
      eventTime3 = millis(); // Đo eventTime3
      Serial.print(eventNum);
      Serial.print(DELIM);
      Serial.print("Gate3"); 
      Serial.print(DELIM);
      Serial.println(eventTime3 / 1000.0, 3); 
      //setMillis(new_val);
      delay(20);
    }
  lastState3 = photoGate3; // Reset lại trạng thái của cổng quang
...
//For NodeMCU
// Cài đặt sẵn các biến 
int IRDetectorPin = 12; // chân để cắm đầu dương của LED thu hồng ngoại
int IRDetectorPin2 = 13; // chân thứ 2 của LED thu thứ 2
int IRDetectorPin3 = 2; // chân thứ 3
int IRLEDPin = 5;      // những chân để cắm đầu dương của LED phát hồng ngoại
char DELIM = '\t';      // ký tự để chia cột trong Serial Monitor
